...
export function CardModal({ card, onClose }: CardModalProps) {
  ...
  const handleMentionSelect = (user: any) => {
    ...
    words.pop();
    const newText =
      (words.length > 0 ? words.join(" ") + " " : "") + `@${user.name} `;
                  ...
                  <div className="space-y-2 mb-3">
                    {checklist.map((item) => (
                      ...
                      >
                        ...
                        <span
                          className={`flex-1 min-w-[200px] text-sm break-words ${item.completed ? "text-textSecondary line-through" : "text-textPrimary"}`}
                        >
                          {item.text}
                        </span>
                        <button
                    ...
                    comments.map((comment) => (
                      <div key={comment.id} className="flex gap-3">
                        ...
                        <div className="flex-1">
                          ...
                          <p className="text-sm text-textPrimary mt-1 whitespace-pre-wrap">
                            {comment.text}
                          </p>
                        </div>
                      </div>
                ...
                {(["low", "medium", "high"] as const).map((p) => (
                  <button
                    ...
                    className={`flex-1 py-1.5 rounded-md text-xs font-medium uppercase tracking-wide transition-all ${
                      card.priority === p
                        ? p === "high"
                          ? "bg-red-500/20 text-red-400 border border-red-500/50 dark:text-red-300"
                          : p === "medium"
