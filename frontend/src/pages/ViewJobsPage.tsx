...
export default function ViewJobsPage() {
  ...
  // Group cards by PIC Name (Initialize with all users)
  const groupedCards: Record<string, { userObj: any, cards: any[] }> = {};

  users.forEach((user) => {
    groupedCards[user.name] = { userObj: user, cards: [] };
  });
  ...
  filteredCards.forEach((card: any) => {
    ...
    if (!groupedCards[picName]) {
      groupedCards[picName] = { userObj: picObj, cards: [] };
    }
    groupedCards[picName].cards.push(card);
  });

  const filteredGroups = Object.entries(groupedCards).filter(([picName, group]) => {
    const picMatch = picName.toLowerCase().includes(searchQuery.toLowerCase());
    const hasMatchingCards = group.cards.length > 0; // The cards are already filtered
    return picMatch || hasMatchingCards || searchQuery === '';
  });
  ...
  const priorityColors = {
    ...
    medium:
      "bg-indigo-100 text-indigo-700 dark:bg-indigo-500/20 dark:text-indigo-300 font-bold",
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div>
          <h1 className="text-2xl font-bold text-textPrimary">
            Semua Pekerjaan (View Jobs)
          </h1>
          <p className="text-sm text-textSecondary mt-1">
            Pantau ringkasan dan detail pekerjaan berdasarkan PIC.
          </p>
        </div>
      ...
      {isLoading ? (
        ...
        <div className="flex flex-col items-center justify-center py-20 bg-bgSecondary/50 rounded-2xl border border-borderBase border-dashed">
          <div className="text-textSecondary mb-2">
            Belum ada hasil yang ditemukan
          </div>
        </div>
      ) : (
        <div className="grid grid-cols-1 xl:grid-cols-2 gap-6">
          {filteredGroups.map(
            ([picName, group]: [string, any]) => {
              ...
              const picObj = group.userObj;
              const roleName = picObj?.role?.name;
              const departmentName = picObj?.department?.name;

              // Calculate stats
              const totalJobs = picCards.length;
              const todoCount = picCards.filter(
                (c: any) => c.columnId === "new",
              ).length;
              const progressCount = picCards.filter(
                (c: any) => c.columnId === "progress",
              ).length;
              const doneCount = picCards.filter(
                (c: any) => c.columnId === "done",
              ).length;
              ...
              return (
                <div
                  key={picName}
                  className="bg-bgSecondary border border-borderBase p-5 rounded-3xl shadow-sm flex flex-col h-max transition-all duration-300 hover:shadow-md"
                >
                  ...
                  >
                    <div className="flex items-center gap-4">
                      <div
                        className={`w-12 h-12 shrink-0 rounded-2xl ${getAvatarColor(picName)} flex items-center justify-center text-lg font-bold transform group-hover:scale-105 transition-transform duration-300`}
                      >
                        {picInitials}
                      </div>
                      <div>
                        <h2 className="text-lg font-bold text-textPrimary group-hover:text-indigo-600 transition-colors">
                          {picName}
                        </h2>
                        {(roleName || departmentName) && (
                          <div className="flex flex-wrap items-center gap-2 mt-1.5 mb-3">
                            {departmentName && (
                              <span className="text-[11px] font-bold px-2.5 py-0.5 rounded-md bg-black text-white border border-gray-800 shadow-sm">
                                {departmentName}
                              </span>
                            )}
                            {roleName && (
                              <span className="text-[11px] font-bold px-2.5 py-0.5 rounded-md bg-black text-sky-300 border border-gray-800 shadow-sm">
                                {roleName}
                              </span>
                            )}
                          </div>
                  ...
                  {isExpanded && (
                    <div className="mt-5 pt-5 border-t border-borderBase flex-1 space-y-3 animate-in fade-in slide-in-from-top-2 duration-200">
                      {picCards.map((card: any) => {
                        ...
                        const sColor =
                          statusColors[card.columnId] || statusColors["new"];
                        const statusText =
                          card.columnId === "new"
                            ? "To Do"
                          ...
                          >
                            <div className="flex justify-between items-start gap-3 mb-2">
                              ...
                              <div className="flex gap-2 shrink-0">
                                <span
                                  className={`text-[9px] uppercase font-bold tracking-wider px-1.5 py-0.5 rounded-sm ${pColor}`}
                                >
                                  {card.priority}
                                </span>
                                <span
                                  className={`text-[9px] uppercase font-bold tracking-wider px-2 py-0.5 rounded-full border ${sColor}`}
                                >
                                  {statusText}
                                </span>
                              </div>
                            ...
                            <div className="flex flex-wrap items-center gap-2 mt-3">
                              ...
                              {card.requestDate && (
                                <div
                                  className="text-[11px] font-semibold text-textSecondary bg-bgSecondary border border-borderBase px-2 py-1 rounded-md"
                                  title="Target Tanggal"
                                >
                                  <span className="text-textPrimary mr-1">
                                    Tgt:
                                  </span>
                                  {new Date(
                                    card.requestDate,
